...
const MODELS = [
  ...
  { id: 'gemini-3-flash-preview', name: 'AcminX-Turbo', color: 'bg-blue-500' }
];
...
const App: React.FC = () => {
  ...
  const handleOpenKeyDialog = async () => {
    if (typeof window !== 'undefined' && (window as any).aistudio?.openSelectKey) {
      ...
      // Per instructions, assume success after opening dialog to mitigate race conditions
      setIsKeySelected(true);
      setError(null);
    } else {
      alert("API Key selection bridge is not available. Ensure you are running in the proper environment.");
    }
  };
  ...
  const handleGenerate = async (e?: React.FormEvent, overridePrompt?: string) => {
    ...
    // Check if key is present (either in selection bridge or injected)
    const hasKey = isKeySelected || !!process.env.API_KEY;
    if (!hasKey) {
      setError("API Key Required: Please configure your Gemini API Key before generating code.");
      handleOpenKeyDialog();
      return;
    }
    ...
    try {
      ...
      if (err.message?.includes("Requested entity was not found")) {
        setError("API Key Error: Requested entity not found. Please ensure your selected key is from a paid project.");
        setIsKeySelected(false);
      } else if (err.message?.includes("API Key is missing")) {
        setError("API Key Error: Your session key is missing. Please re-configure it.");
        setIsKeySelected(false);
      } else {
      ...
      <div className="h-screen bg-[#0a0a0a] text-white flex flex-col font-sans selection:bg-white/20 overflow-y-auto scroll-smooth">
        <nav className="h-20 flex-shrink-0 border-b border-white/[0.08] flex items-center justify-between px-6 md:px-8 bg-black/40 backdrop-blur-3xl sticky top-0 z-[100]">
          ...
          <div className="flex items-center gap-4">
            {!isKeySelected && !process.env.API_KEY && (
              <button 
                onClick={handleOpenKeyDialog}
                className="hidden sm:flex items-center gap-2 text-[10px] font-black bg-red-500/10 text-red-400 border border-red-500/20 px-4 py-2 rounded-xl hover:bg-red-500/20 transition-all animate-pulse"
              >
        ...
        <main className="flex-shrink-0 flex flex-col items-center justify-center px-6 pt-24 pb-32 relative min-h-[85vh]">
          ...
          <div className="w-full max-w-3xl relative z-10 group px-4">
            ...
            <div className="mt-6 text-center space-y-2">
              <button 
                onClick={handleOpenKeyDialog}
                className="text-[10px] font-black text-emerald-500/60 hover:text-emerald-400 uppercase tracking-widest transition-all"
              >
                {isKeySelected ? "✓ API Key Configured" : "Click here to Configure API Key"}
              </button>
              <br />
              <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="text-[10px] text-gray-700 hover:text-gray-500 transition-colors uppercase font-black tracking-widest">Billing Documentation</a>
            </div>
          </div>
        </main>
      </div>
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex h-16 border-b border-white/10 bg-[#0d0d0d] items-center px-8 justify-between">
          ...
          <div className="flex items-center gap-6">
            <button onClick={handleOpenKeyDialog} className="text-[10px] font-black text-gray-500 hover:text-white transition-colors flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" strokeWidth={2}/></svg>
              API KEY
            </button>
